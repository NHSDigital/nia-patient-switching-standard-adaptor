...
1. Create a Postgres database, and run the application schema migration script,
   followed by with the SNOMED import script.
...
1. For the translator container, specify the `MHS_BASE_URL` variable pointing to the MHS Oubound created above, and `PS_DAISY_CHAINING_ACTIVE` to "true".
