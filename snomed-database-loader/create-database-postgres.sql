...
WITH RECURSIVE immunisation_heirarchy AS (
    ...
    FROM snomedct.relationship_s r
             JOIN immunisation_heirarchy i ON r.destinationId = i.conceptId
    WHERE r.typeId = '116680003' -- relationshipType (typeId) is 'IsA' (child of)
)
SELECT conceptId
FROM immunisation_heirarchy
UNION
...
FROM (VALUES
     ...
     ) AS conceptIds_not_in_immunisation_heirachy(conceptId);
