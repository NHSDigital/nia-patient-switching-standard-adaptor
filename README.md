...
2. Set following env variables inside `vars.sh` file:
    - PSS_DB_URL: Database URL required to run migrations (for local environment set 'jdbc:postgresql://localhost:5436/patient_switching')
    ...
    - PSS_DB_OWNER_NAME: Username of user used to run migrations.
    - PSS_DB_OWNER_PASSWORD: Password to be set for the user used to run migrations.
